...
from .utils import get_data_path, is_development
...
def _get_db_path():
    ...
    """
    if not is_development(__file__) and "CHESS_ANALYSIS_DB" in os.environ:
        db_path = os.environ["CHESS_ANALYSIS_DB"]
    else:
        db_path = os.path.join(get_data_path(), "chess.db")
    return db_path
